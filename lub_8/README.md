...
* Реализация моделей с геттерами и сеттерами

* Создание HTTP сервера с маршрутизацией

* Использование Jinja2 для отображение данных

* Использоване курсов валют через API Центрального банка России

* Тестирование для корректной работы
...
* Валидация данных - требовалась корректировка проверок входных параметров в сеттерах моделей

* Обработка запросов - необходимость самостоятельной реализации парсинга URL и query-параметров

* Интеграция внешнего API - обработка неоднородных форматов данных

* Конфигурация шаблонизатора - настройка корректных путей для загрузки шаблонов


Реализация архитектуры MVC

Применение архитектурного паттерна Model-View-Controller позволило

* Четко разделить ответственность между компонентами приложения

* Обеспечить модульность и поддерживаемость кода

* Упростить тестирование отдельных компонентов системы

* Модели содержат бизнес-логику, представления отвечают за отображение данных, контроллер осуществляет координацию между ними.
...
* Разработка HTTP-серверов на базе стандартных библиотек Python

* Использование шаблонизатора Jinja2 для динамической генерации веб-страниц

* Интеграция внешних API с обработкой исключительных ситуаций

* Реализация MVC-архитектуры в клиент-серверных приложениях
